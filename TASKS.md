...
- [x] **Geometry с реальным полигоном** — демо-полигоны в `zn.asset`, отдача GeoJSON через `to_shape().__geo_interface__`, тест на Polygon в ответе ✅
- [x] **Модуль `search`** — `GET /api/v1/search` с фильтрами (q, region, area_min/max, category, status) + пагинация (limit/offset) + total, 14 тестов ✅
